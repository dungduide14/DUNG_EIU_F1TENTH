    ...
    def laser_callback(self, msg: LaserScan):
        ...
        if self.state != self.prev_state: # if robot change its state
            ...
            if self.state == State.WARNING:
                ...
                self.zones.markers[0].color.a = 1.0   # WARNING sáng
                self.zones.markers[1].color.a = 0.2   # DANGER mờ

            elif self.state == State.DANGER:
                is_safety_stop.data = True
                self.zones.markers[0].color.a = 0.0   # WARNING tắt
                self.zones.markers[1].color.a = 1.0   # DANGER sáng

            else:  # State.FREE
                is_safety_stop.data = False
                self.zones.markers[0].color.a = 0.2   # Cả hai mờ
