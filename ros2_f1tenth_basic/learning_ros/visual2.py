...
class SafetyStop(Node):
    def __init__(self):
        ...
        self.declare_parameter("warning_distance", 1.0) # When an obstacle is closer than 60cm to the robot, its velocity will be reduced
        ...
        self.zones_pub = self.create_publisher(MarkerArray,"zones_Dung",10)
        ...
        text_zone = Marker()
        text_zone.header.frame_id = "base_link"
        # text_zone.header.stamp = self.get_clock().now().to_msgs()
        text_zone.id = 2
        text_zone.type = Marker.TEXT_VIEW_FACING
        text_zone.action = Marker.ADD
        text_zone.scale.z = 0.5
        # text_zone.scale.x = self.warning_distance * 2
        # text_zone.scale.y = self.warning_distance * 2
        text_zone.color.r = 1.0
        text_zone.color.g = 1.0
        text_zone.color.b = 0.0
        text_zone.color.a = 1.0
        text_zone.pose.position.z = 1.5
        text_zone.pose.position.x = -1.0
        text_zone.pose.orientation.w = 1.0
        text_zone.text = "EIU"
        ...
        self.zones.markers = [danger_zone, text_zone]
    ...
    def laser_callback(self, msg: LaserScan):
        ...
        if self.state != self.prev_state: # if robot change its state
            ...
            if self.state == State.DANGER:
                ...
                self.zones.markers[0].color.a = 1.0   # DANGER sáng
